    ...
    for i = 1:3
        for j = 1:3
            ...
            function Integrand(x,y)
                α*((x-t.vertices[i])*(y-s.vertices[j])/(2*t.volume*2*s.volume))*G(x,y)+
                β*((2*2)/(2*t.volume*2*s.volume))*G(x,y)
            end

            z[i,j] = sauterschwabintegral(s, t, Integrand, accuracy)
...
function momintegrals!(op::MWDoubleLayer3D, g::RTRefSpace, f::RTRefSpace,
    ...
    function u(x,y)
        G(x,y)*(γ*((x-y).^(-0.5))-((x-y)/(norm(x-y))^0.5))
    end
    ...
    for i = 1:3
        for j = 1:3
            ...
            function Integrand(x,y)
                (x-t.vertices[i])'*cross(u(x,y) , y-s.vertices[j]))
            end
