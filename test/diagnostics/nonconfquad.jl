...
@testitem "Non-conf mesh: cuboid, vectorial, regular" tags=[:diagnostics] begin
    using CompScienceMeshes
    ...
    function meshbox(;h)
        geofile = pkgdir(BEAST) * "/test/assets/twoboxes.geo"
        G10 = meshgeo(geofile; physical="G10", h=h)
        G20 = meshgeo(geofile; physical="G20", h=h)
        G12 = meshgeo(geofile; physical="G12", h=h)

        G20.vertices = G10.vertices
        G12.vertices = G10.vertices

        G0, S1 = CompScienceMeshes.union([-G10, -G20], Val{:submeshes})
        G1, S2 = CompScienceMeshes.union([G10, G12], Val{:submeshes})
        G2, S3 = CompScienceMeshes.union([G20, -G12], Val{:submeshes})
        ∂Ω = [G0,G1,G2]
        Γ = [S1,S2,S3]
        return ∂Ω[3]
    end
...
@testitem "Non-conf mesh: cuboid, vectorial, local" tags=[:diagnostics] begin
    using CompScienceMeshes
    ...
    A12 = assemble(op, X1, X2)
    A11 = assemble(op, X1, X1)
    A22 = assemble(op, X2, X2)

    @show r12 = dot(u11, A12*u22)
    @show r11 = dot(u11, A11*u21)
    @show r22 = dot(u12, A22*u22)

    @test abs(r12 - r11) < 1e-5
    @test abs(r12 - r22) < 1e-5 
end
