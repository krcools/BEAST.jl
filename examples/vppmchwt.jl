...
Γ = [boundary(tetmeshsphere(1.0,0.5))]
#Γ = [Γ1]#,Γ2,Γ3]
Tree = [0]#,0,0] # give index in which material is

HOM = [1]#,2] #indices of homogeneous domains (without free space)
HomPars = Dict(0=>(ϵ0,μ0),1=>(ϵ0*2,μ0*2),2=>(ϵ0*2,μ0*2))#,3=>(ϵ0*3,μ0))#index --> (eps,mu)
...
MZprec =  inv(Matrix(Gxy)')*Matrix(Pyy)*inv(Matrix(Gxy))*Matrix(Zs)
